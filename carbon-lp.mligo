(* An FA2 contract corresponding to the contract's LP token *)
...
type storage = {
    // address of the c4dex contract
    c4dex_contract : address ; 
...
let rec mint_tokens (param, storage : mint * storage) : result =
    if Tezos.sender <> storage.c4dex_contract then (failwith error_FA2_NOT_OPERATOR : result) else 
...
let rec burn_tokens (param, storage : burn * storage) : result = 
    if Tezos.sender <> storage.c4dex_contract then (failwith error_PERMISSIONS_DENIED : transfer) else 
    ...
    | hd :: tl ->
        ...
        let fa2_ownerbalance = 
            ...
            | Some fa2_ownerprev_balance -> fa2_ownerprev_balance)
        in
        if fa2_ownerbalance - fa2_amt < 0 then (failwith error_FA2_INSUFFICIENT_BALANCE : result) else 
        let new_fa2_ownerbalance = abs(fa2_ownerbalance - fa2_amt) in
        let new_ledger = Big_map.update (fa2_owner, fa2_token_id) (Some new_fa2_ownerbalance) storage.ledger in
        let storage = {storage with ledger = new_ledger} in 
